...
int main()
{
    ...
    if (debug) {
        ...
        mainMenu->addItem("Выполнить вариант 30"); //9
        ...
        while (resultSelectedItem != exitInt) {
            ...
            case 8:
                ...
                break;
            case 9:
                /* Вариант 30 Распечатать всех студентов, у которых за все время обучения не более 25 %
                     оценок а) 3 б) 3 и 4 в) 5 г) 3 и 5 д) 4 и 5 Варианты а - д выбираются по желанию пользователя.
                     Предусмотреть распечатывать лиц определенного пола.
                ce->setLabel("Введите начальный год рождения для интервала выборки. ");
                startYear = ce->setDataInt(1900, 2021);
                ce->setLabel("Введите конечный год рождения для интервала выборки. ");
                ...
                //endYear = 2005;
                sdb1->DataBase.clear();
                cout << "Сортировка по полу : 1 - Сортировать по М; 0 - сортировать по Ж; 2 - не важно" << endl;
                int ch;
                cin >> ch;
